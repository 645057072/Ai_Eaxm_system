...
def upgrade() -> None:
    if not has_table("exam_service_record"):
        op.create_table(
            "exam_service_record",
    ...
    safe_create_index("ix_exam_service_record_enterprise_id", "exam_service_record", ["enterprise_id"], unique=False)
    safe_create_index("ix_exam_service_record_exam_no", "exam_service_record", ["exam_no"], unique=False)


def downgrade() -> None:
    if has_table("exam_service_record"):
        if has_index("exam_service_record", "ix_exam_service_record_exam_no"):
            op.drop_index("ix_exam_service_record_exam_no", table_name="exam_service_record")
        if has_index("exam_service_record", "ix_exam_service_record_enterprise_id"):
